...
class FrontendController extends Controller
{
  public function home() {
    $data = [
        ...
        'testimonials' => \App\Models\Product::latest()->take(6)->get(),
        'collections' => CollectionBanner::first(),
        'setting' => GeneralSetting::first(),
        'latestThreeBlogs' => Blog::latest()->take(3)->get(),
        // 'pageViews' => \App\Models\PageView::where('page', 'home')->first()?->count ?? 0,
    ];
...
public function detail($slug, Request $request)
{
    ...
    $suggestedBlogsQuery = Blog::where('id', '!=', $blogs->id)
        ...
        ->latest()
        ->paginate(2); // 2 per page, as in the original code

    if ($request->ajax() && $request->input('section') === 'suggested_blogs') {
        return view('front.default.suggested_blogs', compact('suggestedBlogsQuery'))->render();
    }

    return view('front.default.blog_details', compact('blogs', 'tags', 'latestBlogs', 'comments', 'suggestedBlogsQuery'));
}
