...
<style>
    ...
    .easyzoom {
        ...
        display: inline-block;
        width: 100%;
        overflow: visible !important; /* Ensure zoom overlay isn't clipped */
    }
    .easyzoom img {
        width: 100%;
        height: auto;
    ...
    .easyzoom--overlay .easyzoom-flyout {
        position: absolute;
        z-index: 1000;
        background: #fff; /* Ensure zoom window is visible */
    }
</style>
                                ...
                                <li data-target="#carouselExampleIndicators" data-slide-to="{{ $index }}" class="{{ $index == 0 ? 'active' : '' }}">
                                    @php
                                        $thumbUrl = Str::startsWith($gallery->image, ['http://', 'https://'])
                                            ...
                                            : asset($gallery->image);
                                    @endphp
                                    <img src="{{ $thumbUrl }}" alt="Thumbnail {{ $index + 1 }}" style="width: 60px; height: 60px; object-fit: cover;">
                                </li>
                            @endforeach
                        </ol>
                                ...
                                <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                    <div class="easyzoom easyzoom--overlay">
                                        ...
                                        <a href="{{ $imageUrl }}" class="image-link" data-image="{{ $imageUrl }}">
                                            <img class="d-block w-100 product-image" src="{{ $imageUrl }}" alt="Slide {{ $index + 1 }}" style="max-width:100%;height:auto;" />
                                        </a>
                                    </div>
                                </div>
                            @endforeach
...
<!-- Fallback for EasyZoom if CDN fails -->
<script>
    if (typeof jQuery.fn.easyZoom === 'undefined') {
        console.warn('EasyZoom CDN failed, attempting to load local fallback');
        document.write('<script src="{{ asset('assets/js/easyzoom.js') }}"><\/script>');
    }
</script>
...
(function($) {
    $(document).ready(function () {
        ...
        // Initialize EasyZoom for desktop
        function initEasyZoom() {
            if (window.innerWidth >= 768) {
                if ($.fn.easyZoom) {
                    console.log('Initializing EasyZoom'); // Debug
                    // Destroy existing EasyZoom instances
                    if ($easyzoom) {
                        $easyzoom.each(function() {
                            $(this).data('easyZoom').teardown();
                        });
                    }
                    // Initialize new EasyZoom instances for active slide
                    $easyzoom = $('.carousel-item.active .easyzoom').easyZoom({
                        loadingNotice: 'Loading image...',
                        errorNotice: 'The image could not be loaded',
                        preventClicks: true
                    });
                    // Verify image URLs
                    $('.easyzoom img').each(function() {
                ...
                // Destroy EasyZoom for mobile devices
                if ($easyzoom) {
                    $easyzoom.each(function() {
                        $(this).data('easyZoom').teardown();
                    });
        ...
        // Run EasyZoom on load and resize
        initEasyZoom();
        $(window).on('resize', initEasyZoom);

        // Reinitialize EasyZoom on carousel slide change
        $('#carouselExampleIndicators').on('slid.bs.carousel', function() {
            if (window.innerWidth >= 768 && $.fn.easyZoom) {
                console.log('Reinitializing EasyZoom on slide change'); // Debug
                // Destroy existing EasyZoom instances
                if ($easyzoom) {
                    $easyzoom.each(function() {
                        $(this).data('easyZoom').teardown();
                ...
                // Initialize EasyZoom for the active slide
                $easyzoom = $('.carousel-item.active .easyzoom').easyZoom({
                    loadingNotice: 'Loading image...',
                    errorNotice: 'The image could not be loaded',
                    preventClicks: true
